...
if( $action == "Add Defendant" || $action == "Edit Defendant" )
{
	...
	$defendant->setcourtFileNumber($_POST["courtcase"]);
...
if( $action == "Delete Defendant" )
{
  $defendant = new Defendant();
  $defendant->getFromID( $_GET["id"] );
  if( $user_programID == $defendant->getProgramID() )
    if( $defendant->deleteDefendant() )
      $user->addEvent("Defendant: ".$action, $defendant->getDefendantID() );
...
{
  var_dump($_POST);
  //expunge the defendant if all requirements are checked
  if( $_POST["order"] == "on" && $_POST["letter"] == "on" && 
      $_POST["case"] == "on" && $_POST["defendant-evaluation"] == "on" && 
      $_POST["parent-evaluation"] == "on" && $_POST["workshop"] == "on") {
        $defendant = new Defendant();
        $defendant->setDefendantID( $_POST["defendantID"] );
        if( $program->expunge < 4 ) {
          echo "test";
          $defendant->expungeDefendant( $program->expunge );
        }
      }
