...
if( isset($id) && !$defendant->compareProgramID( $id, $user_programID) ) {
	...
	if( isset($id) )
	{
			...
			$courtFileNumber = $defendant->getcourtFileNumber();
...
jQuery(function($) {	
	...
	$('#physical-location').css("display","none");
	$('#mailing-location').css("display","none");
	$('#school-location').css("display","none");
	$('#guardian-plocation-1').css("display","none");
	$('#guardian-mlocation-1').css("display","none");
	...
	$('#citation-location').css("display","none");
	...
	$('#defendant-personal-submit').attr ( 'disabled', true );
...
<div id="control-header">	
	...
	<div class="right">
      ...
      <? } else { ?>
			<button id="delete-defendant" value="process.php?action=Delete+Defendant&id=<? echo $id  ?>">Delete Defendant</button>
  ...
  <table>
      ...
      <td width="50%" valign="top">
        <table width="100%">
          ...
          <? } ?>
          <tr>
            <td width="125">Court File #:</td>
            <td><input type="text" name="courtcase" size="10" value="<? echo $courtFileNumber ?>" /></td>
          </tr>
          <tr>
            <td>Agency Case #:</td>
